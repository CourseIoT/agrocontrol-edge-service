...
@sensor_api.route('/api/v1/sensor_records', methods=['POST'])
def create_sensor_record():
    ...
    try:
        ...
        record = sensor_record_service.create_and_save_sensor_record(
            device_id, api_key, temperature, humidityAir, humidityFloor, levelWaterOk, bombWaterOk, created_at)
        return jsonify({
            ...
            "created_at": record.created_at.isoformat() + "Z" }), 201
    except KeyError:
